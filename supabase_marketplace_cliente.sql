-- ============================================================
-- DOMIU MAGDALENA - MARKETPLACE CLIENTE
-- Tablas: negocios, productos, pedidos_cliente, detalle_pedido_cliente
-- RLS permisivas para desarrollo
-- Datos de ejemplo: 5 negocios, 20 productos
-- ============================================================
...
CREATE TABLE IF NOT EXISTS pedidos_cliente (
  ...
  estado TEXT NOT NULL DEFAULT 'recibido' CHECK (estado IN ('recibido','preparacion','asignado','camino','entregado','cancelado')),
  repartidor_id UUID REFERENCES profiles(id),
  created_at TIMESTAMPTZ DEFAULT now(),
  updated_at TIMESTAMPTZ DEFAULT now()
);

-- 4. DETALLE PEDIDO CLIENTE
...
-- Negocios: lectura pública, solo admin escribe
...
DROP POLICY IF EXISTS "negocios_insert_admin" ON negocios;
CREATE POLICY "negocios_insert_admin" ON negocios FOR INSERT WITH CHECK (true);
DROP POLICY IF EXISTS "negocios_update_admin" ON negocios;
CREATE POLICY "negocios_update_admin" ON negocios FOR UPDATE USING (true);

-- Productos: lectura pública, solo admin escribe
...
CREATE POLICY "productos_insert_admin" ON productos FOR INSERT WITH CHECK (true);
DROP POLICY IF EXISTS "productos_update_admin" ON productos;
CREATE POLICY "productos_update_admin" ON productos FOR UPDATE USING (true);

-- Pedidos cliente: todos pueden insertar/leer/actualizar (desarrollo)
...
-- Detalle pedido: todos pueden insertar/leer (desarrollo)
...
-- Productos para Pizzeria Napoli (negocio 1)
...
-- Productos para Super Tienda Don Pepe (negocio 2)
...
-- Productos para Licores El Buen Gusto (negocio 3)
...
-- Productos para Drogueria Salud Total (negocio 4)
...
-- Productos para Sushi Master (negocio 5)
