...
async function requireAdmin() {
  ...
  if (!ADMIN_ROLES.includes(result.session.profile.role)) {
    return { error: 'Acceso denegado', session: null };
  }
...
export async function createBusinessCompleteAction(input: z.infer<typeof createSchema>) {
  ...
  if (!owner || owner.deleted_at || owner.status !== 'active') {
    return { error: 'El propietario no existe o no está activo' };
  }
  if (!['customer', 'merchant'].includes(owner.role)) {
    return { error: 'El usuario seleccionado no puede ser propietario' };
  }
  ...
  try {
    ...
    if (businessError || !business) {
      throw new Error('No se pudo crear el negocio: ' + (businessError?.message || ''));
    }
    ...
    if (addressError || !address) {
      throw new Error('No se pudo crear la dirección: ' + (addressError?.message || ''));
    }
    ...
    if (profileError) {
      throw new Error('No se pudo actualizar el propietario: ' + profileError.message);
    }
    ownerUpdated = true;
    ...
    await serverAudit.logAction(
      ...
      'businesses',
      business.id,
      { ownerId: data.ownerId, slug, addressId: address.id },
    );
    ...
    if (businessId) {
      ...
      await supabase.from('business_addresses').delete().eq('business_id', businessId);
...
export async function updateBusinessCompleteAction(input: z.infer<typeof updateSchema>) {
  ...
  const { data: current, error: currentError } = await supabase
    .from('businesses')
    .select('id, name, description, cuisine_type, business_type, phone, email, website, is_verified, updated_at')
  ...
  const { data: currentAddress, error: currentAddressError } = await supabase
    .from('business_addresses')
    .select('id, street_address, city, latitude, longitude, phone, updated_at')
    .eq('id', data.addressId)
    .eq('business_id', data.businessId)
    .maybeSingle();

  if (currentAddressError) {
    return { error: 'No se pudo consultar la dirección: ' + currentAddressError.message };
  }
  ...
  await serverAudit.logAction(
    ...
    auth.session.user.email,
    auth.session.profile.role,
    'update_business_complete',
    'businesses',
    data.businessId,
    { before: current, addressBefore: currentAddress, after: data },
  );
