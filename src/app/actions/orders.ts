...
export async function createOrderAction(input: z.infer<typeof createOrderSchema>) {
  try {
    ...
    if (result.session.user.id !== parsed.data.customerId) return { error: 'No autorizado para crear pedidos para otro usuario' };
    ...
    if (!addressId) return { error: 'No se pudo crear la dirección de entrega' };
    ...
    return { error: err instanceof Error ? err.message : 'Error desconocido al crear pedido' };
  }
}
...
export async function updateOrderStatusAction(orderId: string, status: string) {
  try {
    ...
    if (session.profile.role === 'customer') return { error: 'Clientes no pueden actualizar estados de pedidos' };
    ...
    return { error: err instanceof Error ? err.message : 'Error desconocido al actualizar estado' };
  }
}
...
export async function assignCourierAction(orderId: string, courierId: string) {
  try {
    ...
    const { session } = result;
    if (session.profile.role === 'customer' || session.profile.role === 'courier') return { error: 'No autorizado para asignar repartidores' };
    ...
    return { error: err instanceof Error ? err.message : 'Error desconocido al asignar repartidor' };
  }
}
