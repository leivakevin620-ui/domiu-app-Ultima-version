...
import {
  ...
  type DomiClientMessage,
  type DomiNavigationLink,
...
function browserContext(cart: ReturnType<typeof useCart>) {
  return {
    ...conversationContext(),
...
function normalizeConversation(value: unknown): DomiConversation | null {
  if (!value || typeof value !== 'object') return null;
  const row = value as Record<string, unknown>;
  if (typeof row.id !== 'string' || !isConversationStatus(row.status)) return null;

  const now = new Date().toISOString();
  return {
    id: row.id,
    title: typeof row.title === 'string' && row.title.trim() ? row.title.trim().slice(0, 120) : 'Nueva conversación',
    status: row.status,
    summary: typeof row.summary === 'string' ? row.summary.slice(0, 2_000) : '',
    activeGoal: typeof row.activeGoal === 'string' ? row.activeGoal.slice(0, 500) : null,
    lastMessageAt: typeof row.lastMessageAt === 'string' ? row.lastMessageAt : now,
...
function normalizeConversations(value: unknown): DomiConversation[] {
  if (!Array.isArray(value)) return [];
  return value
    .map(normalizeConversation)
    .filter((conversation): conversation is DomiConversation => Boolean(conversation))
    .slice(0, 100);
}
...
function statusIcon(status: ConversationStatus) {
  ...
  if (status === 'paused') return <Clock3 className="h-4 w-4" />;
...
export function DomiAssistantStable() {
  const { profile, isLoading, retrySession } = useAuth();
  const cart = useCart();
  ...
  const [messages, setMessages] = useState<DomiClientMessage[]>([]);
  ...
  const conversationBlocked = useMemo(
    () => Boolean(currentConversation && currentConversation.status !== 'active'),
    [currentConversation],
  );

  const handleRequestFailure = useCallback(
    (cause: unknown, fallback: string) => {
      if (cause instanceof DomiRequestError && cause.status === 401) {
        void retrySession();
      }
  ...
  const fetchConversations = useCallback(async () => {
    ...
    try {
      const data = await requestDomiJson<ConversationListResponse>('/api/domi/conversations?limit=50');
      const rows = normalizeConversations(data.conversations);
  ...
  const loadConversation = useCallback(
    async (id: string) => {
      ...
      setError('');
      try {
        const data = await requestDomiJson<ConversationResponse>(
          `/api/domi/conversations/${encodeURIComponent(id)}`,
        );
        const conversation = normalizeConversation(data.conversation);
        if (!conversation) throw new Error('La conversación recibida no es válida.');
        setConversationId(conversation.id);
        setCurrentConversation(conversation);
        setMessages(normalizeDomiMessages(data.messages, profile.role));
        setHistoryOpen(false);
      } catch (cause) {
        handleRequestFailure(cause, 'No se pudo cargar la conversación.');
      } finally {
        setHistoryLoading(false);
      }
    },
    [handleRequestFailure, profile],
  );
  ...
  useEffect(() => {
    const handleAdminMenuState = (event: Event) => {
      ...
      const blocked = Boolean(detail?.open);
  ...
  useEffect(() => {
    if (!open || !profile || initializedUserId === profile.id) return;
    let cancelled = false;
    ...
    const initialize = async () => {
      try {
        ...
        if (!cancelled) handleRequestFailure(cause, 'No se pudo recuperar el historial.');
      } finally {
        if (!cancelled) setInitializedUserId(profile.id);
      }
    };
  ...
  }, [fetchConversations, handleRequestFailure, initializedUserId, loadConversation, open, profile]);

  if (!mounted || isLoading || !profile || profile.role === 'guest') return null;
  ...
  const createConversation = async () => {
    ...
    setError('');
    try {
      const data = await requestDomiJson<ConversationResponse>('/api/domi/conversations', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ context: conversationContext() }),
      });
      const conversation = normalizeConversation(data.conversation);
      if (!conversation) throw new Error('No se recibió una conversación válida.');
      setConversationId(conversation.id);
      setCurrentConversation(conversation);
      setMessages([]);
      setMessage('');
      ...
      handleRequestFailure(cause, 'No se pudo iniciar la conversación.');
    } finally {
      setSending(false);
    }
  };
  ...
  const patchConversation = async (id: string, patch: Record<string, string>) => {
    ...
    if (!updated) throw new Error('No se recibió una conversación válida.');
  ...
  const renameCurrent = async () => {
    ...
    try {
      await patchConversation(currentConversation.id, { title: title.slice(0, 120) });
    } catch (cause) {
      handleRequestFailure(cause, 'No se pudo renombrar la conversación.');
    }
  };
  ...
  const archiveCurrent = async () => {
    ...
    try {
      ...
      handleRequestFailure(cause, 'No se pudo archivar la conversación.');
    }
  };
  ...
  const deleteCurrent = async () => {
    ...
    try {
      await requestDomiJson<Record<string, unknown>>(
      ...
      handleRequestFailure(cause, 'No se pudo eliminar la conversación.');
    }
  };
  ...
  const resumeCurrent = async () => {
    ...
    try {
      ...
      handleRequestFailure(cause, 'No se pudo continuar la conversación.');
    } finally {
      setSending(false);
    }
  };
  ...
  const send = async (override?: string) => {
    ...
    if (!text || sending || sendLockRef.current || conversationBlocked) return;
    ...
    setMessages((current) => [
      ...current,
      { id: `local-${Date.now()}`, role: 'user', content: text.slice(0, 2_000) },
    ]);
    setSending(true);
    ...
    try {
      ...
      const assistantMessage =
        typeof data.assistant?.message === 'string'
          ...
          : typeof data.answer === 'string'
            ? data.answer
            : 'Domi respondió, pero el contenido no pudo mostrarse.';

      const suggestedActions = Array.isArray(data.assistant?.suggestedActions)
        ? data.assistant.suggestedActions
      ...
      const navigation: DomiNavigationLink[] = sanitizeDomiNavigation(
        data.assistant?.navigation,
        role,
      );
      ...
      setMessages((current) => [
        ...current,
        {
          ...
          role: 'assistant',
          content: assistantMessage.slice(0, 12_000),
          suggestedActions,
          navigation,
        },
      ]);
      ...
      const updated = rows.find((item) => item.id === nextConversationId);
      if (updated) setCurrentConversation(updated);
    } catch (cause) {
      handleRequestFailure(cause, 'No se pudo conectar con Domi.');
    } finally {
  ...
  const assistant = (
    <>
      <button
        ...
        className={`fixed bottom-[calc(5.25rem+env(safe-area-inset-bottom))] right-4 z-[1400] flex h-14 items-center gap-2 rounded-full bg-[#17191F] px-4 text-white shadow-2xl transition hover:-translate-y-0.5 lg:bottom-5 ${
          open || interfaceBlocked ? 'pointer-events-none scale-90 opacity-0' : ''
        }`}
      >
            ...
            <header className="flex shrink-0 items-center gap-3 border-b bg-[#17191F] px-4 py-3 text-white">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="truncate text-[11px] text-white/65">
                  {ROLE_LABEL[role] || 'Asistente DomiU'} · historial privado
                </p>
              </div>
              <button
                type="button"
                ...
                className={`flex h-10 w-10 items-center justify-center rounded-xl ${
                  historyOpen ? 'bg-[#FFD400] text-[#17191F]' : 'hover:bg-white/10'
                }`}
                  ...
                  <div className="mt-3 flex items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="text-[11px] text-[#7A828D]">
                        Privadas y disponibles al volver a iniciar sesión.
                      </p>
                    </div>
                    <button
                      type="button"
                  ...
                  {!historyLoading &&
                    filteredConversations.map((item) => (
                      <button
                        ...
                        className={`w-full rounded-2xl border bg-white p-4 text-left shadow-sm transition hover:border-[#FFD400] ${
                          conversationId === item.id
                            ? 'border-[#FFD400] ring-2 ring-[#FFD400]/20'
                            : ''
                        }`}
                      >
              ...
              <>
                <div className="flex shrink-0 items-center gap-2 border-b bg-white px-4 py-2.5">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="truncate text-[10px] text-[#8B929C]">
                      {currentConversation
                        ? `${STATUS_LABEL[currentConversation.status]} · ${relativeDate(
                ...
                <div className="min-h-0 flex-1 space-y-3 overflow-y-auto overscroll-contain bg-[#F6F7F9] p-4">
                      ...
                      <p className="mt-2 text-sm leading-relaxed text-[#69717D]">
                        {currentConversation?.summary
                          ...
                          : role === 'customer'
                            ...
                            : 'Puedo orientarte y ejecutar funciones autorizadas según tu perfil. Este hilo quedará guardado.'}
                      </p>
                    </div>
                  )}
                  ...
                  {sending && (
                    <div className="mr-6 flex items-center gap-2 rounded-2xl border bg-white px-4 py-3 text-sm text-[#69717D]">
                      <Loader2 className="h-4 w-4 animate-spin" /> Domi está consultando información autorizada…
                    </div>
                  )}
                  {error && (
                ...
                <footer className="shrink-0 border-t bg-white p-3 pb-[calc(.75rem+env(safe-area-inset-bottom))]">
                  {conversationBlocked ? (
                    ...
                    >
                      <RotateCcw className="h-4 w-4" /> Restaurar y continuar esta conversación
                    </button>
                  ) : (
                  ...
                  {!viewport.keyboardOpen && (
                    <p className="mt-2 text-center text-[9px] text-[#8B929C]">
                      Domi solo usa datos autorizados de tu cuenta y nunca cruza información entre perfiles.
                    </p>
                  )}
                </footer>
              </>
