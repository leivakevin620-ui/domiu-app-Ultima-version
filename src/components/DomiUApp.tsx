...
function formatMoney(value: number) { return new Intl.NumberFormat("es-CO", { style: "currency", currency: "COP", minimumFractionDigits: 0 }).format(value || 0); }
function formatearFecha(fecha: string) { return new Date(fecha).toLocaleString("es-CO", { day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit" }); }

/* ======================== TIPOS ======================== */
type Tab = "panel" | "nuevo-pedido" | "pedidos" | "repartidores" | "locales" | "liquidacion" | "turnos" | "config";
type Pedido = { id: string; codigo: string; cliente: string; telefono: string; direccion: string; barrio: string; notas: string; localId: string; repartidorId: string | null; km: number; envio: number; precio: number; pagoRepartidor: number; empresaRecibe: number; estado: string; liquidado: boolean; cancelado: boolean; createdAt: string; tiempoMin: number; turnoId: string | null; };
...
const pillGreen: CSSProperties = { padding: "8px 16px", borderRadius: 999, background: "#10b981", color: "#ffffff", fontSize: 12, fontWeight: 700 };
const pillYellow: CSSProperties = { padding: "8px 16px", borderRadius: 999, background: "#f59e0b", color: "#ffffff", fontSize: 12, fontWeight: 700 };
...
export default function DomiUApp() {
  ...
  const [currentTime, setCurrentTime] = useState(""); const [googleMapsReady, setGoogleMapsReady] = useState(false);
  const [filtroEstado, setFiltroEstado] = useState(""); const [filtroBusqueda, setFiltroBusqueda] = useState(""); const [pagina, setPagina] = useState(1);

  // Form pedido
  const [pedidoIdEditar, setPedidoIdEditar] = useState<string | null>(null);
  const [cliente, setCliente] = useState(""); const [telefono, setTelefono] = useState("");
  const [direccion, setDireccion] = useState(""); const [barrio, setBarrio] = useState("");
  const [notas, setNotas] = useState(""); const [localId, setLocalId] = useState("");
  const [repartidorId, setRepartidorId] = useState(""); const [km, setKm] = useState("");
  const [tiempoMin, setTiempoMin] = useState(0); const [envio, setEnvio] = useState("");
  const [precioManual, setPrecioManual] = useState(""); const [addressSuggestions, setAddressSuggestions] = useState<AddressRecord[]>([]);
  ...
  const [repNombre, setRepNombre] = useState(""); const [repTelefono, setRepTelefono] = useState("");
  const [repVehiculo, setRepVehiculo] = useState(""); const [repPlaca, setRepPlaca] = useState(""); const [repDocumento, setRepDocumento] = useState("");

  // Form local
  const [locEditId, setLocEditId] = useState<string | null>(null);
  const [locNombre, setLocNombre] = useState(""); const [locDireccion, setLocDireccion] = useState(""); const [locTelefono, setLocTelefono] = useState("");

  // Form config & turno
  const [configTarifaBase, setConfigTarifaBase] = useState("2500"); const [configCostoPorKm, setConfigCostoPorKm] = useState("1200");
  const [configPorcentajeRep, setConfigPorcentajeRep] = useState("55"); const [configWhatsapp, setConfigWhatsapp] = useState("");
  const [configCompanyName, setConfigCompanyName] = useState("DomiU Magdalena");
  const [turnoNotas, setTurnoNotas] = useState("");
  const [pasteText, setPasteText] = useState("");
  const [cancelarId, setCancelarId] = useState<string | null>(null); const [cancelarRazon, setCancelarRazon] = useState("");
  ...
  useEffect(() => {
    ...
    updateTime(); const interval = setInterval(updateTime, 30000); return () => clearInterval(interval);
  }, []);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (dP) setPedidos(dP.map((p: any) => ({ id: p.id, codigo: p.codigo, cliente: p.cliente, telefono: p.telefono, direccion: p.direccion, barrio: p.barrio, notas: p.notas || "", localId: p.local_id || "", repartidorId: p.repartidor_id, km: Number(p.km) || 0, envio: p.envio || 0, precio: p.precio || 0, pagoRepartidor: p.pago_repartidor || 0, empresaRecibe: p.empresa_recibe || 0, estado: p.estado || "Pendiente", liquidado: !!p.liquidado, cancelado: !!p.cancelado, createdAt: p.created_at, tiempoMin: p.tiempo_min || 0, turnoId: p.turno_id || null })));
      ...
      if (dA) setAddressHistory(dA.map((a: any) => ({ id: a.id, cliente: a.cliente, direccion: a.direccion, barrio: a.barrio || "Desconocido", km: Number(a.km) || 0, tiempoMin: a.tiempo_min || 0, envio: a.envio || 0 })));
      if (dC && dC.length > 0) {
        const cfg = dC[0];
        setAppConfig({ id: cfg.id, tarifaBase: cfg.tarifa_base || 2500, costoPorKm: cfg.costo_por_km || 1200, porcentajeRepartidor: cfg.porcentaje_repartidor || 55, whatsappNumber: cfg.whatsapp_number || "", companyName: cfg.company_name || "DomiU Magdalena" });
        setConfigTarifaBase(String(cfg.tarifa_base || 2500)); setConfigCostoPorKm(String(cfg.costo_por_km || 1200));
        setConfigPorcentajeRep(String(cfg.porcentaje_repartidor || 55)); setConfigWhatsapp(cfg.whatsapp_number || ""); setConfigCompanyName(cfg.company_name || "DomiU Magdalena");
      }
      if (dT) {
        ...
        setTurnoActivo(activo ? { id: activo.id, openedAt: activo.opened_at, closedAt: activo.closed_at, notas: activo.notas || "", activo: true, createdAt: activo.created_at } : null);
      }
    } catch (e: any) { fail("Error al cargar: " + (e.message || "")); } finally { setLoading(false); }
  }, [user, fail]);
  ...
  const calcularRuta = useCallback(() => {
    ...
    if (!local) return;
    const saved = addressHistory.find((h) => h.cliente.trim().toLowerCase() === cliente.trim().toLowerCase() && h.direccion.trim().toLowerCase() === direccion.trim().toLowerCase());
    ...
    new g.DistanceMatrixService().getDistanceMatrix({ origins: [local.direccion], destinations: [direccion], travelMode: g.TravelMode.DRIVING, unitSystem: g.UnitSystem.METRIC }, (resp: any, status: string) => {
      setDistanceLoading(false);
      if (status !== "OK") { fail("Error Google Maps."); return; }
      const el = resp.rows?.[0]?.elements?.[0];
      if (!el || el.status !== "OK") { fail("Ruta no encontrada."); return; }
      setKm(String(Math.round((el.distance.value / 1000) * 100) / 100));
      setTiempoMin(Math.max(1, Math.round(el.duration.value / 60)));
      setEnvio(String(calcularEnvio(el.distance.value / 1000)));
    });
  }, [localId, direccion, cliente, locales, addressHistory, fail]);

  useEffect(() => { const t = setTimeout(() => { if (direccion.trim() && localId) calcularRuta(); }, 600); return () => clearTimeout(t); }, [direccion, localId, googleMapsReady, cliente, calcularRuta]);
  useEffect(() => { const q = direccion.trim().toLowerCase(); if (!q || !cliente.trim()) { setAddressSuggestions([]); return; } setAddressSuggestions(addressHistory.filter((h) => h.cliente.trim().toLowerCase() === cliente.trim().toLowerCase() && h.direccion.trim().toLowerCase().includes(q))); }, [cliente, direccion, addressHistory]);

  /* ======================== FUNCIONES LOGICA ======================== */
  const guardarPedido = async (e: FormEvent) => {
    e.preventDefault(); setError("");
    const kmVal = Number(km);
    if (!cliente.trim() || !direccion.trim() || Number.isNaN(kmVal) || kmVal <= 0) { fail("Datos incompletos."); return; }
    if (repartidorId) { const rep = repartidores.find((r) => r.id === repartidorId); if (rep && rep.estado !== "Disponible") { fail(`El repartidor "${rep.nombre}" está "${rep.estado}".`); return; } }
    let precioFinal: number; let pagoRep: number; let empRec: number;
    if (precioManual.trim() && Number(precioManual) > 0) { precioFinal = Number(precioManual); if (precioFinal > 8000) { empRec = Math.round(precioFinal * 0.4); pagoRep = precioFinal - empRec; } else { const base = calcularTarifas(kmVal); const ratio = precioFinal / base.precio; pagoRep = Math.round(base.pagoRepartidor * ratio); empRec = precioFinal - pagoRep; } }
    else { const t = calcularTarifas(kmVal); precioFinal = t.precio; pagoRep = t.pagoRepartidor; empRec = t.empresaRecibe; }
    const envioVal = Number(envio) || calcularEnvio(kmVal);
    const tiempoVal = tiempoMin || 10 + Math.round(kmVal * 4);
    let turnoId: string | null = turnoActivo?.id || null;
    if (!turnoId) { const { data } = await supabase.from("turnos").select("id").eq("activo", true).limit(1); if (data?.length) { turnoId = data[0].id; setTurnoActivo({ id: data[0].id, openedAt: "", closedAt: null, notas: "", activo: true, createdAt: "" }); } }
    let nuevoCodigo = pedidoIdEditar ? (pedidos.find((p) => p.id === pedidoIdEditar)?.codigo || `DOM-${String(pedidos.length + 1).padStart(4, "0")}`) : `DOM-${String(pedidos.length + 1).padStart(4, "0")}`;
    const payload: any = { codigo: nuevoCodigo, cliente: cliente.trim(), telefono: telefono.trim(), direccion: direccion.trim(), barrio: barrio.trim() || "Desconocido", notas: notas.trim() || null, local_id: localId || null, repartidor_id: repartidorId || null, km: kmVal, envio: envioVal, precio: precioFinal, pago_repartidor: pagoRep, empresa_recibe: empRec, estado: pedidoIdEditar ? (pedidos.find((p) => p.id === pedidoIdEditar)?.estado || "Pendiente") : (repartidorId ? "Asignado" : "Pendiente"), liquidado: pedidoIdEditar ? (pedidos.find((p) => p.id === pedidoIdEditar)?.liquidado || false) : false, cancelado: false, cancelado_razon: null, cancelado_at: null, tiempo_min: tiempoVal, turno_id: turnoId, user_id: user?.id };
    try {
      if (pedidoIdEditar) { await supabase.from("pedidos").update(payload).eq("id", pedidoIdEditar); ok("Actualizado."); } else { await supabase.from("pedidos").insert(payload); ok("Creado."); }
      await supabase.from("address_history").insert({ cliente: cliente.trim(), direccion: direccion.trim(), barrio: barrio.trim() || "Desconocido", km: kmVal, tiempo_min: tiempoVal, envio: envioVal, user_id: user?.id });
      if (repartidorId) await supabase.from("repartidores").update({ estado: "Ocupado" }).eq("id", repartidorId);
      setCliente(""); setTelefono(""); setDireccion(""); setBarrio(""); setNotas(""); setLocalId(""); setRepartidorId(""); setKm(""); setTiempoMin(0); setEnvio(""); setPrecioManual(""); setPedidoIdEditar(null); loadData();
    } catch (e: any) { fail("Error: " + (e.message || "desconocido")); }
  };

  const editarPedido = (p: Pedido) => { setPedidoIdEditar(p.id); setCliente(p.cliente); setTelefono(p.telefono); setDireccion(p.direccion); setBarrio(p.barrio); setNotas(p.notas || ""); setLocalId(p.localId || ""); setRepartidorId(p.repartidorId || ""); setKm(String(p.km)); setTiempoMin(p.tiempoMin); setEnvio(String(p.envio)); setPrecioManual(String(p.precio)); setTab("nuevo-pedido"); };
  const eliminarPedido = async (id: string) => { if (!confirm("¿Eliminar?")) return; const { error: e2 } = await supabase.from("pedidos").delete().eq("id", id); if (e2) { fail(e2.message); return; } ok("Eliminado."); loadData(); };
  const cambiarEstado = async (id: string, estado: string) => { const { error: e2 } = await supabase.from("pedidos").update({ estado }).eq("id", id); if (e2) { fail(e2.message); return; } if (estado === "Entregado") { const pedido = pedidos.find((p) => p.id === id); if (pedido?.repartidorId) await supabase.from("repartidores").update({ estado: "Disponible" }).eq("id", pedido.repartidorId); } loadData(); };
  const cancelarPedido = async () => { if (!cancelarId || !cancelarRazon.trim()) { fail("Ingrese motivo."); return; } const pedido = pedidos.find((p) => p.id === cancelarId); if (pedido?.repartidorId) await supabase.from("repartidores").update({ estado: "Disponible" }).eq("id", pedido.repartidorId); await supabase.from("pedidos").update({ estado: "Cancelado", cancelado: true, cancelado_razon: cancelarRazon.trim(), cancelado_at: new Date().toISOString() }).eq("id", cancelarId); ok("Cancelado."); setCancelarId(null); setCancelarRazon(""); loadData(); };
  const copiarTexto = (t: string) => navigator.clipboard.writeText(t).then(() => ok("Copiado."), () => fail("Error"));
  const copiarEmpresa = (p: Pedido) => { const loc = locales.find((l) => l.id === p.localId); const rep = repartidores.find((r) => r.id === p.repartidorId); copiarTexto(`Pedido: ${p.codigo}\nCliente: ${p.cliente}\nBarrio: ${p.barrio}\nDir: ${p.direccion}\nLocal: ${loc?.nombre || "-"}\nRep: ${rep?.nombre || "-"}\nTarifa: ${formatMoney(p.precio)}\nEstado: ${p.estado}`); };
  const enviarWhatsApp = (p: Pedido) => { const phone = appConfig?.whatsappNumber || ""; if (!phone) { fail("Configure WhatsApp."); return; } const msg = `*${appConfig?.companyName || "DomiU"}* Pedido: ${p.codigo}, Cliente: ${p.cliente}, Dir: ${p.direccion}, Rep: ${repartidores.find((r) => r.id === p.repartidorId)?.nombre || "-"}, Estado: ${p.estado}`; window.open(`https://wa.me/57${phone}?text=${encodeURIComponent(msg)}`, "_blank"); };
  const enviarWhatsAppRepartidor = (p: Pedido) => { const rep = repartidores.find((r) => r.id === p.repartidorId); if (!rep?.telefono) { fail("Rep sin teléfono."); return; } const msg = `*Pedido ${p.codigo}*\nCliente: ${p.cliente}\nDir: ${p.direccion}\nTu pago: ${formatMoney(p.pagoRepartidor)}`; window.open(`https://wa.me/57${rep.telefono}?text=${encodeURIComponent(msg)}`, "_blank"); };
  const enviarWhatsAppCliente = (p: Pedido) => { if (!p.telefono) { fail("Cliente sin teléfono."); return; } const msg = `Hola ${p.cliente}, tu pedido *${p.codigo}* está: *${p.estado}*.`; window.open(`https://wa.me/57${p.telefono}?text=${encodeURIComponent(msg)}`, "_blank"); };
  const abrirTurno = async () => { await supabase.from("turnos").insert({ user_id: user?.id, notas: turnoNotas.trim() || null }); ok("Turno abierto."); setTurnoNotas(""); loadData(); };
  const cerrarTurno = async (id: string) => { await supabase.from("turnos").update({ closed_at: new Date().toISOString(), activo: false }).eq("id", id); ok("Turno cerrado."); loadData(); };
  const guardarConfig = async (e: FormEvent) => { e.preventDefault(); const payload: any = { tarifa_base: Number(configTarifaBase), costo_por_km: Number(configCostoPorKm), porcentaje_repartidor: Number(configPorcentajeRep), whatsapp_number: configWhatsapp.trim(), company_name: configCompanyName.trim() }; try { if (appConfig?.id) await supabase.from("app_config").update(payload).eq("id", appConfig.id); else await supabase.from("app_config").insert({ ...payload, user_id: user?.id }); ok("Guardado."); loadData(); } catch (e: any) { fail("Error: " + e.message); } };
  const guardarRepartidor = async (e: FormEvent) => { e.preventDefault(); if (!repNombre.trim()) return; try { const payload: any = { nombre: repNombre.trim(), telefono: repTelefono.trim(), vehiculo: repVehiculo.trim() || null, placa: repPlaca.trim() || null, documento: repDocumento.trim() || null }; if (repEditId) await supabase.from("repartidores").update(payload).eq("id", repEditId); else await supabase.from("repartidores").insert({ ...payload, estado: "Disponible", user_id: user?.id }); ok("Guardado."); setRepNombre(""); setRepTelefono(""); setRepVehiculo(""); setRepPlaca(""); setRepDocumento(""); setRepEditId(null); loadData(); } catch (e: any) { fail("Error: " + e.message); } };
  const eliminarRepartidor = async (id: string) => { if (!confirm("¿Desactivar?")) return; await supabase.from("repartidores").update({ activo: false }).eq("id", id); ok("Desactivado."); loadData(); };
  const guardarLocal = async (e: FormEvent) => { e.preventDefault(); if (!locNombre.trim()) return; try { if (locEditId) await supabase.from("locales").update({ nombre: locNombre.trim(), direccion: locDireccion.trim(), telefono: locTelefono.trim() }).eq("id", locEditId); else await supabase.from("locales").insert({ nombre: locNombre.trim(), direccion: locDireccion.trim(), telefono: locTelefono.trim(), user_id: user?.id }); ok("Guardado."); setLocNombre(""); setLocDireccion(""); setLocTelefono(""); setLocEditId(null); loadData(); } catch (e: any) { fail("Error: " + e.message); } };
  const eliminarLocal = async (id: string) => { if (!confirm("¿Desactivar?")) return; await supabase.from("locales").update({ activo: false }).eq("id", id); ok("Desactivado."); loadData(); };
  const liquidarRepartidor = async (repId: string) => { await supabase.from("pedidos").update({ liquidado: true }).eq("repartidor_id", repId).eq("estado", "Entregado").eq("liquidado", false); ok("Liquidado."); loadData(); };
  const parsePasteText = () => { if (!pasteText.trim()) return; const lines = pasteText.split(/\r?\n/).filter((l) => l.trim()); let c = "", t = "", d = "", b = ""; for (const line of lines) { const low = line.toLowerCase(); if (/^(cliente|nombre|client)\s*[:\-]/.test(low)) c = line.replace(/^[^:\-]*[:\-]\s*/, "").trim(); else if (/^(tel|phone|telefono)\s*[:\-]/.test(low)) t = line.replace(/^[^:\-]*[:\-]\s*/, "").trim(); else if (/^(dir|dirección|direccion)\s*[:\-]/.test(low)) d = line.replace(/^[^:\-]*[:\-]\s*/, "").trim(); else if (/^(barrio|zona)\s*[:\-]/.test(low)) b = line.replace(/^[^:\-]*[:\-]\s*/, "").trim(); } if (!c && lines.length) c = lines[0].replace(/^[^:\-]*[:\-]\s*/, "").trim(); if (!t) { const m = pasteText.match(/\b(3\d{9})\b/); if (m) t = m[1]; } if (!d) d = lines.find((l) => /\d/.test(l))?.replace(/^[^:\-]*[:\-]\s*/, "").trim() || ""; if (c) { setCliente(c); setTelefono(t); setDireccion(d); setBarrio(b); ok("Datos extraídos."); } else fail("No se encontró cliente."); setPasteText(""); };
  const exportToExcel = () => { if (!pedidos.length) return; const ws = XLSX.utils.json_to_sheet(pedidos.map(p => ({ ...p, local: locales.find(l => l.id === p.localId)?.nombre, rep: repartidores.find(r => r.id === p.repartidorId)?.nombre }))); const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, "Pedidos"); XLSX.writeFile(wb, `DomiU_Pedidos_${new Date().toISOString().split("T")[0]}.xlsx`); ok("Excel descargado."); };
  const exportToPDF = () => { if (!pedidos.length) return; const doc = new jsPDF(); doc.text(`Reporte Pedidos - ${appConfig?.companyName || "DomiU"}`, 14, 15); (doc as any).autoTable({ startY: 25, head: [["Código", "Cliente", "Dir", "Barrio", "Rep", "Precio", "Estado"]], body: pedidos.map(p => [p.codigo, p.cliente, p.direccion, p.barrio, repartidores.find(r => r.id === p.repartidorId)?.nombre || "-", formatMoney(p.precio), p.estado]), styles: { fontSize: 7 }, headStyles: { fillColor: [250, 204, 21] } }); doc.save("Pedidos.pdf"); ok("PDF descargado."); };

  const pedidosDet = useMemo(() => pedidos.filter((p) => !p.cancelado).map((p) => ({ ...p, localNombre: locales.find((l) => l.id === p.localId)?.nombre || "Sin local", repartidorNombre: repartidores.find((r) => r.id === p.repartidorId)?.nombre || "Sin asignar" })), [pedidos, locales, repartidores]);
  const pedidosFiltrados = useMemo(() => { let r = pedidosDet; if (filtroEstado) r = r.filter((p) => p.estado === filtroEstado); if (filtroBusqueda) { const q = filtroBusqueda.toLowerCase(); r = r.filter((p) => p.cliente.toLowerCase().includes(q) || p.codigo.toLowerCase().includes(q)); } return r; }, [pedidosDet, filtroEstado, filtroBusqueda]);
  ...
  const liquidacionRep = useMemo(() => repartidores.map((rep) => { const ent = pedidos.filter((p) => p.repartidorId === rep.id && p.estado === "Entregado"); return { rep, pedidos: ent.length, debeEmpresa: ent.reduce((s, p) => s + p.empresaRecibe, 0), ganancia: ent.reduce((s, p) => s + p.pagoRepartidor, 0), liquidado: ent.length > 0 && ent.every((p) => p.liquidado) }; }), [pedidos, repartidores]);
    ...
    <main style={pageStyle}>
      <aside style={sidebarStyle}>
        ...
        <nav style={{ display: "grid", gap: 8 }}>
          {([
            { key: "panel", label: "Panel", icon: "📊" }, { key: "nuevo-pedido", label: "Crear Pedido", icon: "➕" },
            { key: "pedidos", label: "Pedidos", icon: "📦" }, { key: "repartidores", label: "Repartidores", icon: "🚴" },
            { key: "locales", label: "Locales", icon: "🏠" }, { key: "liquidacion", label: "Liquidaciones", icon: "💰" },
            { key: "turnos", label: "Turnos", icon: "⏰" }, { key: "config", label: "Configuración", icon: "⚙️" },
          ] as const).map((item) => (
            <button key={item.key} type="button" style={{ width: "100%", padding: "14px 18px", borderRadius: 16, border: tab === item.key ? "1px solid #facc15" : "1px solid rgba(255,255,255,0.08)", background: tab === item.key ? "#111827" : "rgba(255,255,255,0.03)", color: tab === item.key ? "#ffffff" : "#cbd5e1", textAlign: "left", cursor: "pointer", fontSize: 15, fontWeight: 600, boxShadow: tab === item.key ? "0 10px 30px rgba(250,204,21,0.18)" : "none" }} onClick={() => setTab(item.key)}>
        ...
        {tab === "panel" && (
          <>
            ...
            <div style={panelCard}>
              ...
              {repartidores.length === 0 ? <p style={{ color: "#64748b", textAlign: "center", padding: 24 }}>No hay repartidores.</p> :
            ...
            {liquidacionRep.length > 0 && (
              <div style={panelCard}>
                ...
                <table style={tableStyle}>
                  ...
                  <tbody>{liquidacionRep.map((item) => (<tr key={item.rep.id}><td style={tdStyle}>{item.rep.nombre}</td><td style={tdStyle}>{item.pedidos}</td><td style={tdStyle}>{formatMoney(item.debeEmpresa)}</td><td style={tdStyle}>{formatMoney(item.ganancia)}</td><td style={tdStyle}><span style={item.liquidado ? pillGreen : pillYellow}>{item.liquidado ? "Liquidado" : "Pendiente"}</span></td></tr>))}</tbody>
                </table>
              </div>
            )}
          </>
          ...
          <div style={panelCard}>
            ...
            <div style={{ ...miniCard, marginBottom: 20 }}>
              <div style={{ flex: 1 }}>
                ...
                <textarea style={{ ...inputStyle, minHeight: 80, resize: "vertical", fontFamily: "monospace", fontSize: 13 }} placeholder={"Ejemplo:\nCliente: Juan Pérez\nTeléfono: 3001234567\nDirección: Calle 123"} value={pasteText} onChange={(e) => setPasteText(e.target.value)} />
            ...
            <form onSubmit={guardarPedido} style={{ display: "grid", gap: 16 }}>
              ...
              {addressSuggestions.length > 0 && (
                <div style={suggestionList}>
                  {addressSuggestions.map((item) => (<button key={item.id} type="button" style={suggestionItem} onClick={() => { setDireccion(item.direccion); setBarrio(item.barrio); setKm(String(item.km)); setTiempoMin(item.tiempoMin); setEnvio(String(item.envio)); setAddressSuggestions([]); }}>{item.direccion} · {item.barrio}</button>))}
                </div>
              )}
              ...
              <label style={labelStyle}>Notas<input style={inputStyle} placeholder="Observaciones (opcional)" value={notas} onChange={(e) => setNotas(e.target.value)} /></label>
              <label style={labelStyle}>Local<select style={inputStyle} value={localId} onChange={(e) => setLocalId(e.target.value)}><option value="">Selecciona local</option>{locales.map((l) => <option key={l.id} value={l.id}>{l.nombre}</option>)}</select></label>
              <label style={labelStyle}>Repartidor<select style={inputStyle} value={repartidorId} onChange={(e) => setRepartidorId(e.target.value)}><option value="">Sin asignar</option>{repartidoresDisponibles.map((r) => <option key={r.id} value={r.id}>{r.nombre} ✅</option>)}{repartidores.filter((r) => r.estado !== "Disponible").map((r) => (<option key={r.id} value={r.id} disabled>{r.nombre} ({r.estado})</option>))}</select></label>
              <label style={labelStyle}>Kilómetros *<input style={inputStyle} placeholder="Distancia" type="number" min="0.1" step="0.1" value={km} onChange={(e) => setKm(e.target.value)} required /></label>
              <label style={labelStyle}>Precio manual<input style={inputStyle} placeholder="Dejar vacío para tarifa automática" type="number" min="0" value={precioManual} onChange={(e) => setPrecioManual(e.target.value)} /></label>
              <div style={infoBox}>
                {tarifasPreview && !precioManual.trim() && (
                  <>
                    <p><strong>Tarifa automática: {formatMoney(tarifasPreview.precio)}</strong></p>
              ...
              <div style={{ display: "flex", gap: 12 }}>
                <button type="submit" style={primaryBtn}>{pedidoIdEditar ? "Actualizar" : "Guardar pedido"}</button>
        ...
        {tab === "pedidos" && (
          <div style={panelCard}>
            ...
            <div style={{ overflowX: "auto" }}>
              <table style={tableStyle}>
                <thead><tr><th style={thStyle}>Código</th><th style={thStyle}>Cliente</th><th style={thStyle}>Barrio</th><th style={thStyle}>Local</th><th style={thStyle}>Repartidor</th><th style={thStyle}>Precio</th><th style={thStyle}>Repartidor</th><th style={thStyle}>Empresa</th><th style={thStyle}>Estado</th><th style={thStyle}>Acciones</th></tr></thead>
                <tbody>
                  {pedidosPagina.map((p) => (
                    <tr key={p.id}>
                      ...
                      <td style={tdStyle}><select style={{ padding: "8px 12px", borderRadius: 12, border: "1px solid rgba(255,255,255,0.12)", background: "#09111d", color: "#f8fafc", fontSize: 13 }} value={p.estado} onChange={(e) => cambiarEstado(p.id, e.target.value)}>{ESTADOS_PEDIDO.map((e) => <option key={e} value={e}>{e}</option>)}</select></td>
                      <td style={{ ...actionsCell, ...tdStyle }}>
                        ...
                        <button type="button" style={smallBtn} onClick={() => enviarWhatsApp(p)}>WA emp.</button>
                        <button type="button" style={smallBtn} onClick={() => enviarWhatsAppRepartidor(p)}>WA rep.</button>
                        <button type="button" style={smallBtn} onClick={() => enviarWhatsAppCliente(p)}>WA cli.</button>
        ...
        {cancelarId && (
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9999 }}>
            <div style={{ background: "#0f172a", borderRadius: 24, padding: 32, border: "1px solid rgba(255,255,255,0.1)", maxWidth: 420, width: "90%" }}>
              ...
              <label style={{ ...labelStyle, marginBottom: 16 }}>Motivo *<textarea style={{ ...inputStyle, minHeight: 80, resize: "vertical", fontSize: 14 }} placeholder="Ej: Cliente canceló..." value={cancelarRazon} onChange={(e) => setCancelarRazon(e.target.value)} /></label>
              <div style={{ display: "flex", gap: 12 }}>
                <button type="button" style={{ ...primaryBtn, background: "#ef4444", color: "#fff" }} onClick={cancelarPedido}>Confirmar</button>
        ...
        {tab === "repartidores" && (
          <div style={panelCard}>
            ...
            <form onSubmit={guardarRepartidor} style={{ display: "grid", gap: 16, marginBottom: 24 }}>
              ...
              <label style={labelStyle}>Vehículo<input style={inputStyle} placeholder="Moto..." value={repVehiculo} onChange={(e) => setRepVehiculo(e.target.value)} /></label>
              ...
              <label style={labelStyle}>Documento<input style={inputStyle} placeholder="CC" value={repDocumento} onChange={(e) => setRepDocumento(e.target.value)} /></label>
              <div style={{ display: "flex", gap: 12 }}>
          ...
          <div style={panelCard}>
            ...
            <table style={tableStyle}>
              ...
              <tbody>
                ...
                {liquidacionRep.length === 0 && <tr><td colSpan={6} style={{ textAlign: "center", padding: 24, color: "#64748b" }}>No hay datos.</td></tr>}
              </tbody>
            </table>
          </div>
        )}
          ...
          <div style={panelCard}>
            ...
            {!turnoActivo && (
              <div style={{ ...miniCard, marginBottom: 20, flexDirection: "column", alignItems: "stretch" }}>
                <p style={{ margin: "0 0 12px", color: "#94a3b8" }}>No hay turno activo.</p>
                <label style={{ ...labelStyle, marginBottom: 12 }}>Notas (opcional)<input style={inputStyle} placeholder="Ej: Turno mañana..." value={turnoNotas} onChange={(e) => setTurnoNotas(e.target.value)} /></label>
            ...
            {turnos.length === 0 ? <p style={{ color: "#64748b", textAlign: "center", padding: 24 }}>No hay turnos.</p> :
              <div style={{ display: "grid", gap: 8 }}>
                {turnos.map((t) => { const pedidosTurno = pedidos.filter((p) => p.turnoId === t.id); const totalTurno = pedidosTurno.reduce((s, p) => s + p.precio, 0); return (
        ...
        {tab === "config" && (
          <div style={panelCard}>
            ...
            <form onSubmit={guardarConfig} style={{ display: "grid", gap: 16 }}>
              ...
              <label style={labelStyle}>WhatsApp (sin 57)<input style={inputStyle} placeholder="3001234567" value={configWhatsapp} onChange={(e) => setConfigWhatsapp(e.target.value)} /></label>
              <label style={labelStyle}>Tarifa base<input style={inputStyle} placeholder="2500" type="number" value={configTarifaBase} onChange={(e) => setConfigTarifaBase(e.target.value)} /></label>
              ...
              <div style={infoBox}>
                <p><strong>Nota:</strong> WhatsApp y tarifas configuradas aquí.</p>
              </div>
