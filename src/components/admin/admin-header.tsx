...
export function AdminHeader() {
  ...
  return (
    <header
      className={cn(
        'sticky top-0 z-30 flex min-h-16 min-w-0 items-center justify-between gap-2 border-b border-border/50 bg-background/90 px-3 py-2 backdrop-blur-xl sm:px-6',
      )}
    >
      <div className="min-w-0 flex-1">
        ...
        <p className="truncate text-sm font-bold text-foreground sm:hidden">
          {breadcrumbs.at(-1)?.label || 'Administración'}
        </p>
      </div>
      ...
      <div className="flex shrink-0 items-center gap-1 sm:gap-2">
        ...
        <div className="relative">
          <button
            ...
            className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-primary/10 to-primary/5 text-sm font-medium text-primary ring-1 ring-primary/10 transition-all duration-200 hover:from-primary/20 hover:to-primary/10"
            ...
            <>
              ...
              <div className="absolute right-0 top-full z-50 mt-2 w-[min(14rem,calc(100vw-1rem))] rounded-2xl border border-border bg-card p-2 shadow-dropdown animate-scale-in">
                ...
                <div className="mt-1 space-y-0.5">
                  <button
                    type="button"
                    onClick={() => { router.push('/admin/configuracion'); setMenuOpen(false); }}
