...
export function RevenueLineChart({
  ...
  if (normalized.length === 0) {
    return <p className="py-8 text-center text-sm text-muted-foreground">Sin datos de ingresos</p>;
  }
...
export function OrdersBarChart({ data }: { data: { hour: number; count: number }[] }) {
  if (data.length === 0) {
    return <p className="py-8 text-center text-sm text-muted-foreground">Sin datos de pedidos</p>;
  }
  return (
      ...
      <BarChart data={data}>
        ...
        <Bar dataKey="count" radius={[4, 4, 0, 0]} fill="hsl(var(--primary))">
          {data.map((item, index) => (
            <Cell
              key={`${item.hour}-${index}`}
              fill={item.count > 0 ? 'hsl(var(--primary))' : 'hsl(var(--border))'}
            />
          ))}
        </Bar>
      </BarChart>
    ...
    <ResponsiveContainer width="100%" height={220}>
      <PieChart>
        <Pie
          ...
          paddingAngle={3}
          dataKey="count"
          label={(properties: unknown) =>
            `${(Number((properties as Record<string, unknown>).percent ?? 0) * 100).toFixed(0)}%`
          }
        >
          {data.map((item, index) => (
...
export function CityBarChart({
  ...
  if (data.length === 0) {
    return <p className="py-8 text-center text-sm text-muted-foreground">Sin datos por ciudad</p>;
  }
  return (
    <ResponsiveContainer width="100%" height={200}>
      <AreaChart data={data}>
