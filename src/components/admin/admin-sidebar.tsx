...
const sidebarItems = [
  ...
  { label: 'Crear domicilio', href: '/admin/pedidos/crear', icon: PackagePlus, featured: true },
...
export function AdminSidebar() {
  ...
  React.useEffect(() => {
    ...
    return () => {
      if (mobileOpen) {
        window.dispatchEvent(new CustomEvent('domiu:admin-menu-state', { detail: { open: false } }));
      }
    };
  }, [mobileOpen]);
  ...
  const visibleItems = sidebarItems.filter((item) => {
    const permission = permissionMap[item.label];
    if (!permission) return true;
    if (!profile) return false;
    if (profile.email === 'domiumagdalena@gmail.com') return true;
    return permissionsService.hasPermission(profile.admin_role, permission as Permission, profile.email);
  });
  ...
  const sidebar = (
    <aside
      className={cn(
        'isolate flex h-full w-[min(88vw,20rem)] flex-col border-r border-white/10 bg-gradient-to-b from-[#111419] via-[#15191f] to-[#0d1014] text-white shadow-2xl transition-all duration-300 lg:shadow-none',
    ...
    >
      ...
      >
        ...
        {compact && (
          <Link href="/admin" className="domiu-brand-glow flex h-11 w-11 items-center justify-center rounded-xl bg-[#232831]" aria-label="DomiU Magdalena Admin">
        ...
        <button
          ...
          className="flex h-10 w-10 items-center justify-center rounded-xl border border-white/10 text-slate-300 transition-colors hover:bg-white/10 hover:text-[#FFC400] lg:hidden"
      ...
      <div className={cn('px-3 pt-3', compact && 'lg:px-2')}>
        <div className="relative">
          ...
          <input
            ...
            placeholder={compact ? '' : 'Buscar opción...'}
            className={cn(
              'h-11 w-full rounded-xl border border-white/10 bg-white/[0.06] text-base text-white placeholder:text-slate-500 transition-all duration-200 focus-visible:border-[#FFC400]/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#FFC400]/20 lg:h-9 lg:text-sm',
              compact ? 'lg:px-2' : 'pl-10 pr-3',
            )}
          />
        </div>
      </div>
      ...
      <nav className="flex-1 overflow-y-auto overscroll-contain p-3 pb-6">
        <p className={cn('mb-2 px-3 text-[10px] font-black uppercase tracking-[0.18em] text-slate-500', compact && 'lg:hidden')}>
      ...
      <div className="shrink-0 border-t border-white/10 p-3">
        <div className="mb-3 rounded-xl border border-[#FFC400]/20 bg-[#FFC400]/[0.07] p-3">
          <div className="flex items-center gap-2">
            ...
            {!compact && <span className="text-xs font-bold text-[#FFD34D]">DomiU Enterprise</span>}
          </div>
          {!compact && <p className="mt-1 text-[10px] text-slate-400">Control operativo y financiero</p>}
        </div>
        <button
          onClick={() => void logout()}
          className={cn(
  ...
  return (
    <>
      ...
      <div
        className={cn(
          'fixed inset-y-0 left-0 z-[2000] transition-transform duration-300 ease-out lg:hidden',
