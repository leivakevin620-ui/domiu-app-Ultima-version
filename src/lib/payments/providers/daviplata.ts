...
export class DaviplataProvider implements IPaymentProvider {
  readonly name: PaymentProviderName = 'daviplata';
  ...
  async createPaymentIntent(request: PaymentRequest): Promise<PaymentResult> {
    ...
    const intent: PaymentIntent = {
      ...
      status: 'authorized',
      provider: this.name,
      providerIntentId: `dv_${randomUUID().replace(/-/g, '').slice(0, 24)}`,
    ...
    return {
      success: true,
      intent,
      redirectUrl: `https://daviplata.com/pay/${intent.providerIntentId}`,
      requiresAction: true,
    };
  }
  ...
  async capturePayment(intentId: string, amount?: number): Promise<PaymentResult> {
    ...
    const txn: PaymentTransaction = {
      ...
      currency: 'COP',
      fee: 0,
      netAmount: amount ?? intent.amount,
      status: 'paid',
      provider: this.name,
