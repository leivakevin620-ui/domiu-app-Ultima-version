...
const CONFIG: PaymentProviderConfig = {
  enabled: true,
  testMode: true,
  supportedMethods: ['credit_card', 'debit_card', 'pse', 'nequi', 'daviplata', 'cash'],
  feePercentage: 3.5,
  feeFixed: 400,
  maxAmount: 999999999,
  minAmount: 100,
...
export class PayUProvider implements IPaymentProvider {
  ...
  async createPaymentIntent(request: PaymentRequest): Promise<PaymentResult> {
    ...
    const intent: PaymentIntent = {
      ...
      status: 'authorized',
      provider: this.name,
      providerIntentId: `pu_${randomUUID().replace(/-/g, '').slice(0, 24)}`,
    ...
    if (request.methodType === 'pse' && request.returnUrl) {
      return {
        success: true,
        intent,
        redirectUrl: `https://checkout.payu.com/pse/${intent.providerIntentId}`,
        requiresAction: true,
      };
    }
    if (request.methodType === 'cash') {
      return {
        success: true,
        intent,
        redirectUrl: `https://checkout.payu.com/cash/${intent.providerIntentId}`,
        requiresAction: true,
      };
    }
  ...
  async capturePayment(intentId: string, amount?: number): Promise<PaymentResult> {
    ...
    const txn: PaymentTransaction = {
      ...
      currency: 'COP',
      fee: Math.round((amount ?? intent.amount) * 0.035) + 400,
      netAmount: (amount ?? intent.amount) - Math.round((amount ?? intent.amount) * 0.035) - 400,
      status: 'paid',
      provider: this.name,
