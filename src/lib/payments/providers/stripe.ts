...
export class StripeProvider implements IPaymentProvider {
  ...
  async createPaymentIntent(request: PaymentRequest): Promise<PaymentResult> {
    ...
    const intent: PaymentIntent = {
      ...
      status: 'authorized',
      provider: this.name,
      providerIntentId: `pi_${randomUUID().replace(/-/g, '').slice(0, 24)}`,
  ...
  async capturePayment(intentId: string, amount?: number): Promise<PaymentResult> {
    ...
    const txn: PaymentTransaction = {
      ...
      currency: 'COP',
      fee: Math.round((amount ?? intent.amount) * 0.029) + 300,
      netAmount: (amount ?? intent.amount) - Math.round((amount ?? intent.amount) * 0.029) - 300,
      status: 'paid',
      provider: this.name,
