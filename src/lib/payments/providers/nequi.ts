...
export class NequiProvider implements IPaymentProvider {
  readonly name: PaymentProviderName = 'nequi';
  ...
  async createPaymentIntent(request: PaymentRequest): Promise<PaymentResult> {
    ...
    const intent: PaymentIntent = {
      ...
      status: 'authorized',
      provider: this.name,
      providerIntentId: `nq_${randomUUID().replace(/-/g, '').slice(0, 24)}`,
    ...
    return {
      success: true,
      intent,
      redirectUrl: `https://nequi.com/pay/${intent.providerIntentId}`,
      requiresAction: true,
    };
  }
