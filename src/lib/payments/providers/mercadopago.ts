...
const CONFIG: PaymentProviderConfig = {
  enabled: true,
  testMode: true,
  supportedMethods: ['credit_card', 'debit_card', 'pse'],
  feePercentage: 3.2,
  feeFixed: 350,
  maxAmount: 999999999,
  minAmount: 100,
...
export class MercadoPagoProvider implements IPaymentProvider {
  ...
  async createPaymentIntent(request: PaymentRequest): Promise<PaymentResult> {
    ...
    const intent: PaymentIntent = {
      ...
      status: 'authorized',
      provider: this.name,
      providerIntentId: `mp_${randomUUID().replace(/-/g, '').slice(0, 24)}`,
  ...
  async capturePayment(intentId: string, amount?: number): Promise<PaymentResult> {
    ...
    const txn: PaymentTransaction = {
      ...
      currency: 'COP',
      fee: Math.round((amount ?? intent.amount) * 0.032) + 350,
      netAmount: (amount ?? intent.amount) - Math.round((amount ?? intent.amount) * 0.032) - 350,
      status: 'paid',
      provider: this.name,
