...
export class PSEProvider implements IPaymentProvider {
  readonly name: PaymentProviderName = 'pse';
  ...
  async createPaymentIntent(request: PaymentRequest): Promise<PaymentResult> {
    ...
    const intent: PaymentIntent = {
      ...
      status: 'authorized',
      provider: this.name,
      providerIntentId: `pse_${randomUUID().replace(/-/g, '').slice(0, 24)}`,
    ...
    return {
      success: true,
      intent,
      redirectUrl: `https://pse.com/pay/${intent.providerIntentId}`,
      requiresAction: true,
    };
  }
  ...
  async capturePayment(intentId: string, amount?: number): Promise<PaymentResult> {
    ...
    const txn: PaymentTransaction = {
      ...
      currency: 'COP',
      fee: Math.round((amount ?? intent.amount) * 0.018) + 250,
      netAmount: (amount ?? intent.amount) - Math.round((amount ?? intent.amount) * 0.018) - 250,
      status: 'paid',
      provider: this.name,
