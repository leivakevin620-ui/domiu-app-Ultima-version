...
CREATE INDEX idx_profiles_role_status ON profiles(role, status) WHERE deleted_at IS NULL;
CREATE INDEX idx_businesses_owner_active ON businesses(owner_id, is_active) WHERE deleted_at IS NULL;
CREATE INDEX idx_products_business_available ON products(business_id, status) WHERE deleted_at IS NULL;
CREATE INDEX idx_orders_customer_status_date ON orders(customer_id, status, created_at DESC) WHERE deleted_at IS NULL;
CREATE INDEX idx_orders_business_status_date ON orders(business_id, status, created_at DESC) WHERE deleted_at IS NULL;
